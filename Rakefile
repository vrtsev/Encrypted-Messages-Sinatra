...
namespace :generate do
  ...
  task :migration do
    ...
    filename = '%s_%s.rb'.format [Time.zone.now.strftime('%Y%m%d%H%M%S'), ENV['NAME'].underscore]
  ...
  task :spec do
    ...
    name     = ENV['NAME'].camelize
    filename = '%s_spec.rb'.format ENV['NAME'].underscore
...
namespace :db do
  ...
  task :create_test do
    puts 'Creating database testasign_test if it doesnt exist...'
    exec('createdb test_asign_test')
  end
